...
def printStats():
	...
	for seqID, sequence in sequence_dict.items():
		stats_dict[seqID] = generateDNAStats(sequence)
	print("%s%10s%10s%13s%13s%12s%12s%12s%12s%13s%13s%13s%13s"%('SeqID','#Exons','#Introns','AvgExonLen','AvgIntronLen','%A in Exon','%C in Exon','%G in Exon','%T in Exon','%A in Intron','%C in Intron','%G in Intron','%T in Intron'))
	for seqId, stats in stats_dict.items():
		print ('%s'%seqID, end='') 
		print ('%8d %9d %12.2f %12.2f %12.2f %12.2f %12.2f %12.2f %12.2f %12.2f %12.2f %12.2f'%stats)
